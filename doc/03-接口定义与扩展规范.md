...
- `activeProjection === configuredProjection` 始终成立
- `projection: 'globe'` 创建纯 VerticalPerspective 管道，无内部 Mercator 回退
- `projection: 'mercator'` 创建纯 Mercator 管道（行为不变）
- 禁止混合过渡状态和双 Transform 同步

### 9.3 使用场景

- **legacy-hybrid**（默认）：保持向后兼容，允许渐进式迁移
- **strict**：启用严格分离，消除混合模式开销，适用于新项目或已完成迁移的场景
